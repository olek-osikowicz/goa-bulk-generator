...
from goamapper.drawer import drawAreas, drawCircut, drawWays
...
class Generator:
    def __init__(self, poster: Poster, overwrite: bool = False) -> None:
        ...
        self.template = None
        self.dir_path = RENDERS_DIR / self.poster.dir_name
        self.svg_file_path = self.dir_path / f"{self.poster.poster_name}.svg"
        self.png_file_path = self.dir_path / f"{self.poster.poster_name}.png"
    ...
    def generate_svg(self):
        ...
        if self.png_file_path.exists() and not self.overwrite:
            ...
            log.info(
                f"{self.poster.dir_name} in {self.poster.poster_name} doesn't exist yet"
            )
    ...
    def create_template(self):
        ...
        # create frame
        tf = dw.Rectangle(*self.frame_dims, id="frame", fill=t.map_frame.fill)
        ...
        for layer_name, layer_info in self.poster.map_layers.items():
            ...
            match layer_name:
                ...
                case "water":  # water must be specially treated
                    self.map_content.append(
                        drawAreas(
                            self.fetcher.get_waterGDF(), id="water", fill=layer_info["fill"]
                        )
                    )
                ...
                case "circut":
                    ...
                    self.map_content.append(drawCircut(gdf, layer_info["style"]))

                case _:
                    gdf = self.fetcher.get_osmGDF(
